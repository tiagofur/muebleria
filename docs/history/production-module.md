...
| **Estado de Planta** (tablero proyectos × sectores, visible a todos los roles) | ✅ F093 | `PlantBoardScreen`, ruta `/plant-board` |
...
| **Reorg de menú** (PRODUCCIÓN = Dashboard · Órdenes* · Producción · Embarques · Instalaciones) | ✅ 2026-08-18 | `docs/history/roadmap-screens/00-overview.md` §2b |
...
| **Board por obra en estaciones** (métricas por proceso para el operador) | 📐 plan aprobado | JD 2026-08-18 — ver D10 + `docs/history/roadmap-screens/03-fabrica.md` §v2 |
...
(`/orders/:id/:tab`). M2 puede consolidar dashboards — ver `docs/history/roadmap-screens/00-overview.md`.
...
estación. Ver `docs/history/roadmap-screens/00-overview.md` §2b + §M2.

**Fuente ejecutable de rutas y acceso:** [`apps/web/src/routes.ts`](../apps/web/src/routes.ts)
→ `NAV_PATHS` y `roleCanAccessNav` son la autoridad. Este documento describe
ownership y roadmap; no inventa paths ni permisos. En el nav actual,
`orders` → `/orders` es la cola/hub OP y `production` → `/production` son las
estaciones. Consolidar o retirar Órdenes en M2 sigue **planned**.

**Aliases deprecated:** `/fabrica` → `/production`, `/planta` →
`/plant-board`, `/embarques` → `/shipments`, `/instalaciones` →
`/installations`; `fabric` → `production`; y `production` como alias de
Órdenes → `orders`. No usarlos en enlaces, código nuevo ni pantallas nuevas.
...
| D5 | Nesting nativo | **Revisada 2026-08-20:** habilitado por decisión explícita del dueño del producto (sesión de plan DXF). Motor MaxRects + export DXF neutro (F124–F126). El Optimizer.xlsx sigue siendo la verdad de corte para **sierra**; el DXF es la salida para **CNC nesting**. Post-procesadores de marca siguen congelados (D6). |
...
Decisiones D9 + D10. Spec de pantalla: `docs/history/roadmap-screens/03-fabrica.md` §v2.
...
| 2026-08-17 | **F093 (Fase 1 del plan):** visibilidad para todos — franja de procesos en detalle de cotización, chip de sector en cola, tablero **Estado de Planta** (`/plant-board`, todos los roles). |
