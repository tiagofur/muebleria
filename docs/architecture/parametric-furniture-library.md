...
- **Parameter Contract (`FurnitureParameter`):** Dynamic, typed parameters (`number`, `string`, `boolean`, `enum`) with explicit units (`mm`, `deg`, `count`), constraints (`min`, `max`, `step`, `options`), default values, stable `sortOrder`, and semantic groupings (`dimension`, `configuration`, `style`, `hardware`, `metadata`). Every non-metadata value declares a versioned authoritative binding; metadata explicitly has no physical consumer. Legacy `widthMm`/`heightMm`/`depthMm` are reserved projections from module columns rather than a second persisted source.
