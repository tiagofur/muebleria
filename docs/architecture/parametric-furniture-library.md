...
- **Parameter Contract (`FurnitureParameter`):** Dynamic, typed parameters (`number`, `string`, `boolean`, `enum`) with explicit units (`mm`, `deg`, `count`), constraints (`min`, `max`, `step`, `options`), default values, and semantic groupings (`dimension`, `configuration`, `style`, `hardware`).
