...
13. [Candidato final r5 y gate pre-campo](13_final_r5_candidate_gate.md): #793 — publica `ptx-cadmatic-4@r5` + `granete-ptx@1.4.0` + receiver `HPP250-CAD4-R5-CANDIDATE`, conecta la autoridad de labels frozen a la ruta productiva (hard gates sin fallback), spec-checked bytes, catálogo r5 y el field pack final de revisión (NOT_TESTED/notClaimed).
14. `packages/excel/src/ptx/`: compiler, validator, serializer, parser y verifier.
15. `contracts/machineOutputCatalog.contract.json`: catálogo compartido con paridad
...
**No enviar otro PTX al cliente todavía.** Plan r5 completo: strict spec preflight (#788 ✓), PARTS_INF/UDI/etiquetas/identidad CNC (#789 ✓), receiver profile HPP250/CAD4 (#790 ✓), CUTS diferencial (#791 ✓), field pack con `/RESULT` (#792 ✓) e integración final (#793 ✓ — ver `13_final_r5_candidate_gate.md`). El field pack de revisión quedó generado localmente (`artifacts-local/ptx-r5-final-review/`, no versionado) y PENDIENTE DE REVISIÓN INDEPENDIENTE antes de cualquier envío. La siguiente prueba debe pedir el `.RLT` — el pack ya lo exige en su README — y no otra ronda manual de interpretación de popups.
