...
// AmbientMaterial is a presentation-only floor/wall surface for the 3D scene
// (spec #4150 / design #4151). It deliberately carries NO pricing or BOM
...
// PBR roughness/metalness/clearcoat) plus a surface_type discriminator. The
// nullable numeric fields use *float64 so NULL (unset) stays distinct from 0
// in the JSON contract — the client optionalNum helper relies on that.
...
// TEXT in Postgres and validated client-side against {floor, wall}.
