...
//
// Every method in this family runs inside runInTenantTx: the request's tenant
// transaction when present (the AuthMiddleware boundary), or a dedicated
// pool-safe one otherwise. The three tables behind this file carry FORCE RLS
// for granete_app, so a query that reaches the pool without the
// app.organization_id actor set returns zero rows — existing data would
// masquerade as a business not-found. The boundary wrapper keeps RLS armed
// on every path and turns a missing organization scope into ErrNoOrgScope
// instead of an ambiguous tenant query.
...
func (s *PostgresStore) CreateAgregadoRevision(
	...
	return runInTenantTx(s, ctx, func(ctx context.Context) (*domain.AgregadoRevision, error) {
...
func (s *PostgresStore) GetPublishedAssemblySnapshotByID(ctx context.Context, snapshotID string) (*domain.PublishedAssemblySnapshotRecord, error) {
	return runInTenantTx(s, ctx, func(ctx context.Context) (*domain.PublishedAssemblySnapshotRecord, error) {
		orgID := OrgFromCtx(ctx)
		query := `
...
func (s *PostgresStore) PinDesignRevisionAssemblySnapshot(ctx context.Context, pin *domain.DesignRevisionAssemblySnapshot) error {
	// The pin itself may carry the organization when the caller context does not.
	if OrgFromCtx(ctx) == "" && pin.OrganizationID != "" {
		ctx = WithOrgCtx(ctx, pin.OrganizationID)
	}
	return runInTenantTxErr(s, ctx, func(ctx context.Context) error {
		orgID := OrgFromCtx(ctx)
...
func (s *PostgresStore) ListDesignRevisionAssemblySnapshots(ctx context.Context, projectID, designRevisionID string) ([]domain.DesignRevisionAssemblySnapshot, error) {
	return runInTenantTx(s, ctx, func(ctx context.Context) ([]domain.DesignRevisionAssemblySnapshot, error) {
		query := `
