...
describe('@muebles/ui package surface', () => {
  ...
  it('exports AppShell layout (F017)', () => {
    ...
    expect(APP_NAV_SECTIONS).toHaveLength(7);
    expect(APP_NAV_SECTIONS[0]?.id).toBe('trabajo');
    expect(APP_NAV_SECTIONS[1]?.id).toBe('ventas');
    expect(APP_NAV_SECTIONS[2]?.id).toBe('ingenieria');
    expect(APP_NAV_SECTIONS[3]?.id).toBe('almacen');
    expect(APP_NAV_SECTIONS[4]?.id).toBe('libreria');
    expect(APP_NAV_SECTIONS[5]?.id).toBe('catalogos');
    expect(APP_NAV_SECTIONS[6]?.id).toBe('config');
  });
