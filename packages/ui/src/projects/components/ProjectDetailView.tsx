    ...
    <>
      ...
      <div className="project-detail__body">
        <div className="project-detail__main">
          ...
          {optionGroups.length > 0 && ctx.onUpdateProjectLevelChoices ? (
            <section className="project-detail__section project-level-options" aria-label="Opciones del proyecto" data-testid="project-level-options">
              ...
              <p className="project-editor__hint">
                Defaults de cotización. Cada mueble las hereda salvo que overridees en la línea.
              </p>
                ...
                {optionGroups.map((group) => {
                  ...
                  return (
                    <div key={group.id} className="catalog-form__field">
                      <label htmlFor={`project-level-${group.code}`}>{group.name} ({group.code})</label>
                      <select id={`project-level-${group.code}`} value={project.projectLevelChoices?.[group.code] ?? ''}
                        onChange={(e) => updateProjectLevelChoice(group.code, e.target.value)}
          ...
          {onUpdateMeasureDefaults && !isProjectClosed(project.status) ? (
            (() => {
              const typesInUse = Array.from(new Set(modules.map((m) => m.furnitureType ?? 'inferior'))) as FurnitureType[];
              if (typesInUse.length === 0) return null;
              const labels: Record<FurnitureType, string> = { inferior: 'Inferiores (gabinetes)', superior: 'Superiores (alacenas)', alto: 'Altos (despensas)' };
              return (
                <section className="project-detail__section project-measure-defaults" aria-label="Parámetros de medida del proyecto" data-testid="project-measure-defaults">
                  <div className="project-detail__section-header">
                    <h3 className="project-detail__section-title">Parámetros de medida</h3>
                  </div>
                  <p className="project-editor__hint">
                    Defaults de fondo/alto (mm) por tipo de mueble. Al agregar un mueble se pre-selecciona el preset más cercano; cada línea puede override.
                  </p>
                  <div className="measure-defaults-grid">
                    {typesInUse.map((type) => {
                      const entry = project.measureDefaults?.[type];
                      return (
                        <div key={type} className="measure-defaults-row" data-testid={`project-measure-default-${type}`}>
                          <span className="measure-defaults-row__label">{labels[type]}</span>
                          <div className="measure-defaults-row__inputs">
                            <div className="measure-input">
                              <label htmlFor={`md-${type}-depth`} className="measure-input__label">Fondo (mm)</label>
                              <input id={`md-${type}-depth`} className="measure-input__field" type="number" min={1} step="any" placeholder="Ej. 560"
                                value={entry?.depth ?? ''} onChange={(e) => updateMeasureDefaults(type, 'depth', e.target.value)}
                                data-testid={`project-measure-default-${type}-depth`} />
                            </div>
                            <div className="measure-input">
                              <label htmlFor={`md-${type}-height`} className="measure-input__label">Alto (mm)</label>
                              <input id={`md-${type}-height`} className="measure-input__field" type="number" min={1} step="any" placeholder="Ej. 720"
                                value={entry?.height ?? ''} onChange={(e) => updateMeasureDefaults(type, 'height', e.target.value)}
            ...
            {project.items.length === 0 ? (
              ...
              <div className="project-item-list">
                {project.items.map((item, index) => {
                  ...
                  return (
                    <div key={item.id} className="project-item-card" data-testid={`project-item-${item.id}`}>
                      <div className="project-item-card__header">
                        <h4 className="project-item-card__title">
                          Ítem {index + 1}: {mod ? `${mod.name} — ${mod.code}` : `Mueble desconocido (${item.moduleId})`}
                          {mod?.furnitureType ? (
        ...
        {/* Totals aside */}
        <aside className={previewBlocked || !breakdown ? 'project-totals project-totals--blocked project-totals--sticky' : 'project-totals project-totals--sticky'}
          aria-label="Totales de cotización" aria-live="polite">
          <div className="project-totals__header">
            <div className="project-totals__heading">
              <h3 className="project-totals__title">
                {isProjectClosed(project.status) && project.priceSnapshot ? 'Totales (congelados)' : 'Totales'}
              </h3>
              {isProjectClosed(project.status) && project.priceSnapshot ? (
                <span className="project-totals__frozen-badge" title={`Precios capturados el ${formatIsoDate(project.priceSnapshot.capturedAt)}`}>
                  Precios congelados
                </span>
              ) : null}
            </div>
          ...
          <PricePreviewGate requiredGroupCodes={previewBlocked ? missingGroups : []} optionChoices={{}} groupLabels={groupLabels}
            ...
            {breakdownLoading && !breakdown ? (
              ...
              <dl className="project-totals__grid">
                ...
                <div className="project-totals__sale-row">
                  ...
                  <dd className="project-totals__sale">{formatProjectMoney(breakdown.salePrice, project.currency)}</dd>
                </div>
              </dl>
            ) : (
              <p className="project-totals__empty">
                {project.items.length === 0 ? 'Agregá muebles para ver totales.' : 'No se pudo calcular el desglose con las opciones actuales.'}
              </p>
            )}
          </PricePreviewGate>

          {materialSummary && (materialSummary.materials.length > 0 || materialSummary.hardware.length > 0) ? (
            <section className="project-material-summary" aria-label="Resumen de materiales" data-testid="project-material-summary">
              <h4 className="project-material-summary__title">Resumen de materiales</h4>
              {materialSummary.materials.length > 0 ? (
                <div className="project-material-summary__block">
                  <p className="project-material-summary__label">Tableros · {materialSummary.totalAreaM2.toFixed(3)} m²</p>
                  <ul className="project-material-summary__list">
                    {materialSummary.materials.map((row) => (
                      <li key={row.materialId}>
                        <span className="project-material-summary__name">{row.name}</span>
                        <span className="project-material-summary__meta">
                          {row.areaM2.toFixed(3)} m²
                          {showCosts ? ` · ${formatProjectMoney(row.boardCost, project.currency)}` : ''}
                        </span>
                      </li>
                    ))}
                  </ul>
                  {(() => {
                    const sheets = estimateBoardSheets(materialSummary.materials, catalogs.materials).filter((s) => s.estimatedSheets > 0);
                    if (sheets.length === 0) return null;
                    return (
                      <div className="project-material-summary__block" data-testid="project-sheet-estimate" style={{ marginTop: '0.75rem' }}>
                        <p className="project-material-summary__label">Pliegos estimados</p>
                        <p className="catalog-form__hint" style={{ marginTop: 0 }}>Estimado — nesting real en software de corte</p>
                        <ul className="project-material-summary__list">
                          {sheets.map((s) => (
                            <li key={s.materialId}>
                              <span className="project-material-summary__name">{s.name}</span>
                              <span className="project-material-summary__meta">
                  ...
                  {project.nestingImport && project.nestingImport.rows.length > 0 ? (
                    <div className="project-material-summary__block" data-testid="project-nesting-import" style={{ marginTop: '0.75rem' }}>
                      <p className="project-material-summary__label">Nesting importado</p>
                      <p className="catalog-form__hint" style={{ marginTop: 0 }}>
                        Consumo real ({project.nestingImport.sourceName ?? 'CSV'}) · {new Date(project.nestingImport.importedAt).toLocaleString()}
                      </p>
                      <ul className="project-material-summary__list">
                        {project.nestingImport.rows.map((r) => (
                          <li key={r.materialCode}>
                            <span className="project-material-summary__name">{r.materialCode}</span>
                            <span className="project-material-summary__meta">
                  ...
                  {canMutate && onImportNesting ? (
                    <div style={{ marginTop: '0.75rem' }}>
                      <label className="btn btn--secondary btn--sm" style={{ cursor: 'pointer' }}>
                        ...
                        <input type="file" accept=".csv,text/csv,text/plain" style={{ display: 'none' }} data-testid="project-nesting-file"
                          onChange={(e) => {
                            ...
                            void file.text().then((text) => {
                              ...
                              onImportNesting(project.id, nestingImportFromRows(rows, new Date().toISOString(), file.name));
                            });
                          }} />
                      </label>
                      <p className="catalog-form__hint">Columnas: material_code, sheets_used [, area_m2]</p>
                    </div>
                  ) : null}
                </div>
              ) : null}
              {materialSummary.edges.length > 0 ? (
                <div className="project-material-summary__block">
                  <p className="project-material-summary__label">Cantos · {materialSummary.totalEdgeMl.toFixed(2)} ML</p>
                  <ul className="project-material-summary__list">
                    {materialSummary.edges.map((row) => (
                      <li key={row.edgeBandId}>
                        ...
                        <span className="project-material-summary__meta">
                          ...
                          {showCosts ? ` · ${formatProjectMoney(row.edgeCost, project.currency)}` : ''}
                        </span>
                      </li>
                    ))}
                  </ul>
              ...
              {materialSummary.hardware.length > 0 ? (
                <div className="project-material-summary__block">
                  <p className="project-material-summary__label">Herrajes · {materialSummary.hardware.reduce((s, h) => s + h.quantity, 0)} uds</p>
                  <ul className="project-material-summary__list">
                    {materialSummary.hardware.map((row) => (
                      <li key={row.hardwareId}>
                        <span className="project-material-summary__name">{row.description}</span>
                        <span className="project-material-summary__meta">
                          ×{row.quantity}
                          {showCosts ? ` · ${formatProjectMoney(row.lineCost, project.currency)}` : ''}
                        </span>
                      </li>
                    ))}
                  </ul>
