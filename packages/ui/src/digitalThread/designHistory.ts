...
/**
 * Formats a SHA-256 hash for compact presentation.
 */
export function formatSha256Digest(sha: string, prefixLength = 8): string {
  const trimmed = sha?.trim() || '';
  if (!trimmed) return 'sha256:—';
  const clean = trimmed.toLowerCase().startsWith('sha256:') ? trimmed.slice(7) : trimmed;
  if (clean.length <= prefixLength) return `sha256:${clean}`;
  return `sha256:${clean.slice(0, prefixLength)}…`;
}
