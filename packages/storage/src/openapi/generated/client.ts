...
import type {
  ...
  DesignWorkingMaterialsReconciliation,
...
export abstract class GeneratedGraneteApiClient {
  ...
  prepareProjectDesignDraftUnits(token: string, projectId: string, designId: string, key = this.createIdempotencyKey(), signal?: AbortSignal): Promise<Design> { return this.request("POST", `/projects/${encodeURIComponent(projectId)}/designs/${encodeURIComponent(designId)}/draft-units:prepare`, { schema: "Design", token, idempotencyKey: key, signal }); }
