...
export function moduleToApi(m: Module): Record<string, unknown> {
  return {
    ...
    width_mm: m.externalDims?.width ?? (m as any).widthMm ?? (m as any).width_mm ?? 0,
    height_mm: m.externalDims?.height ?? (m as any).heightMm ?? (m as any).height_mm ?? 0,
    depth_mm: m.externalDims?.depth ?? (m as any).depthMm ?? (m as any).depth_mm ?? 0,
...
export function projectToApi(p: Project): Record<string, unknown> {
  return {
    ...
    sales_organization_id: p.salesOrganizationId ?? '',
    manufacturing_organization_id: p.manufacturingOrganizationId ?? '',
...
export function projectFromApi(raw: Record<string, unknown>): Project {
  ...
  return {
    ...
    organizationId: str(raw.organization_id ?? raw.organizationId) || undefined,
    salesOrganizationId: str(raw.sales_organization_id ?? raw.salesOrganizationId) || undefined,
    manufacturingOrganizationId: str(raw.manufacturing_organization_id ?? raw.manufacturingOrganizationId) || undefined,
