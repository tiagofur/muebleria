...
# If DSN was provided externally (not generated by --fresh-container),
# validate that it points to a test-only allowed database before any test execution.
if [ -z "${GATE_CONTAINER}" ]; then
  echo "[pilot-gate] validando que DSN externo sea una base de test permitida..."
  python3 - "${DSN}" <<'PY_VALIDATE' || {
import sys
from urllib.parse import urlparse

dsn = sys.argv[1].strip() if len(sys.argv) > 1 else ""
if not dsn:
    sys.stderr.write("[pilot-gate] DSN vacío\n")
    sys.exit(1)

try:
    parsed = urlparse(dsn)
except Exception as e:
    sys.stderr.write(f"[pilot-gate] DSN no parseable: {e}\n")
    sys.exit(1)
...
if not path or path == "muebles" or "prod" in path or "prod" in host:
    sys.stderr.write(f"[pilot-gate] DSN externo RECHAZADO por apuntar a base no permitida o producción: host={host} db={path}\n")
    sys.exit(1)
...
is_allowed = path in allowed_exact or any(path.startswith(prefix) for prefix in allowed_prefixes)
if not is_allowed:
    sys.stderr.write(f"[pilot-gate] DSN externo RECHAZADO por no tener nombre de test permitido: host={host} db={path}\n")
    sys.exit(1)
PY_VALIDATE
    echo "[pilot-gate] ❌ DSN externo inválido o no seguro para tests. Abortando antes de emitir queries." >&2
    exit 1
  }
fi
