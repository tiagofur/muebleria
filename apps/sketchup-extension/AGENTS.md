...
| --- | --- | --- | --- |
| Placement preview | `ui/bridges/placement_preview_bridge.rb`: ciclo begin/commit/cancel, anchors y wiring. `host/placement_environment.rb` posee base planes, targets snap y tolerancia geométrica. No crea identidad comercial ni calcula fabricación. | `ui/bridges/placement_preview_bridge.rb`; `host/placement_environment.rb`; `ui/dialog_controller.rb`; `tools/furniture_placement_tool.rb`; `connection/project_furniture.rb` | `test/unit/placement_preview_controller_test.rb` |
