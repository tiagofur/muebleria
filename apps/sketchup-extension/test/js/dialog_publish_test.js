...
function runDialog() {
  ...
  // granete-material-roles.js, the shared runtime state and the #466
  // review controller around the inline script: load them so the publish
  // gate is exercised exactly as in the HtmlDialog.
  ...
  const { media, account, library, configurator, finishSelector, materialRoles, inline } = dialogSources();
