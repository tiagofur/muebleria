...
class PersistenceRoundtripTest < Minitest::Test
  ...
  def test_reconstructs_full_semantic_hierarchy_from_stored_attributes_without_relying_on_geometry
    ...
    assert_equal "kitchen-base-standard", restored_furniture_meta.dig("intent", "furnitureDefinitionId")
    ...
    shelf_components = components.select do |g|
      meta = @store.read(g)
      meta && meta["kind"] == "componentInstance" && meta.dig("intent", "semanticRole")&.start_with?("shelf")
    end
