...
## 2b. Ronda correctiva de revisión (3 blockers)

1. **Aprobación de producción gated server-side**: `approveDesignRevision` acepta ahora un body opcional `{quoteRevisionId}` (contrato `ApproveDesignRevisionRequest`). Con el pin exacto, el storage ejecuta `enforceProductionGates` — el MISMO gate chain del release (baseline aceptada exacta → gate comercial de reconciliación → preflight autoritativo sobre el par exacto) — ANTES de la transición published→approved, rechazando con los mismos 409 tipados (`quote_not_accepted` implícito en `ErrReleaseQuoteNotAccepted`, `reconciliation_conflict`/`commercial_baseline_outdated`, `manufacturing_preflight_blocked` con issues). La forma sin body conserva la transición lifecycle pura para flujos no-productivos. El Web de #502 siempre envía el pin. Proofs: storage (happy path limpio + replay idempotente; cambio comercial sin incorporar → `ReleaseCommercialGateError`; contrato de catálogo estrechado tras el snapshot → `ReleasePreflightBlockedError` SIN delta comercial y la revisión queda published; baseline draft → `ErrReleaseQuoteNotAccepted`; baseline cross-project → error tipado; legacy sin body sigue funcionando) y E2E real (aprobación con Q2 draft → 409 tipado visible; con Q2 accepted → éxito — el par prueba que el pin viaja, pues el path legacy habría aprobado el caso draft).
2. **Pin comercial obligatorio en el release de #502**: sin checkbox — el panel requiere la cotización seleccionada aceptada, el comando siempre envía `quote_revision_id` exacto y el modal muestra «Cotización (base comercial exacta)». El backend conserva el campo opcional para flujos legacy design-only.
