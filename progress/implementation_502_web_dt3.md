...
## 2b. Ronda correctiva de revisión (3 blockers + separación de comandos)

1. **Aprobación de producción como comando propio, siempre gated**: dos conceptos separados con superficies separadas.
   - `approveDesignRevision` (`POST /designs/{designId}/revisions/{revisionId}:approve`, sin body) queda como el lifecycle genérico de diseño (#395: flujos design-first sin baseline comercial). NUNCA representa aprobación de producción.
   - `approveProjectDesignRevisionForProduction` (`POST /projects/{projectId}/designs/{designId}/revisions/{revisionId}:approve-for-production`, body `ApproveDesignRevisionForProductionRequest` con `quoteRevisionId` REQUERIDO) es la aprobación de producción del Digital Thread comercial: siempre ejecuta `enforceProductionGates` — el MISMO gate chain del release (baseline aceptada exacta → gate comercial de reconciliación → preflight autoritativo sobre el par exacto) — ANTES de published→approved, con los mismos 409 tipados (`ErrReleaseQuoteNotAccepted`, `reconciliation_conflict`/`commercial_baseline_outdated`, `manufacturing_preflight_blocked` con issues). NO existe modo skip: sin pin exacto el comando es inválido antes de tocar el store.
   - El Web de #502 usa EXCLUSIVAMENTE el comando de producción. La protección comercial del hilo se mantiene además en el límite del release: un release pineado a quote sobre una revisión aprobada por el lifecycle genérico sigue rechazado por el gate comercial (probado).
   - Proofs: storage (happy path limpio + replay idempotente; cambio comercial sin incorporar → `ReleaseCommercialGateError`; catálogo estrechado tras el snapshot → `ReleasePreflightBlockedError` SIN delta comercial y la revisión queda published; baseline draft/foránea → errores tipados; pin vacío → comando inválido; lifecycle genérico sigue funcionando y su release quote-pineado sigue bloqueado), HTTP (401/403/400 sin body·pin inválido·vendedor/404/forwarding verbatim; el pin vacío no llega al store) y E2E real (aprobación con Q2 draft → 409 tipado visible; con Q2 accepted → éxito).
2. **Pin comercial obligatorio en el release de #502**: sin checkbox — el panel requiere la cotización seleccionada aceptada, el comando siempre envía `quote_revision_id` exacto y el modal muestra «Cotización (base comercial exacta)». El backend conserva el campo opcional de #395 como el flujo legacy DISTINTO design-only (release sin baseline comercial), separado conceptualmente del comando que usa #502; el gate comercial protege todo release que pinee quote, venga la aprobación de donde venga.
3. **Proyección del preflight por capability (server-authoritative)**: `ManufacturingPreflightResult` incorpora `message` (copy business-safe server-computed), `blockedItemCount` e `includesDetail`. Los roles con `RoleCanReleaseProduction` (admin, gerente_produccion, ingeniero) reciben ítems + issues con códigos/diagnósticos; el resto de roles con acceso a proyectos (vendedor, gerente_ventas) reciben veredicto + mensaje + conteo, SIN internals de fabricación. Proofs HTTP: admin/gerente_produccion/ingeniero → detalle verbatim; vendedor → proyección resumida sin leak (`fi-*`, códigos, `parameter`, `furnitureDefinitionId` ausentes del payload).
...
- **Completar modelado**: R2 publicada con FI-C también → Q2/R2 reconcilia 4 synced (el gate comercial exige baseline al día: `quoted_not_modeled` bloquea — verificado por diseño).
- **Gate de aprobación (negative proof real)**: aprobar R2 con Q2 draft seleccionada → 409 tipado «la cotización base no está aceptada» visible en UI, sin success (prueba que el pin exacto viaja con el comando: el path legacy habría aprobado).
- **Aprobación R2 con Q2 accepted pineada** → gates pasan server-side → «R2 aprobada» sólo tras respuesta.
- **Release P1** vía modal (base comercial exacta Q2) → «Liberación #1 creada» fijada a Q2+R2; fila de historial `Q2 + R2` + fingerprint; badge contextual con pins exactos.
