# Issue #669 — [P1][AS3D-3] GLB y representación 3D coherente (entrega parcial: paridad + contrato + renderer + Proyectar)

- Approval: prompt del propietario (2026-09-19) autoriza implementar #669 completo; prerrequisitos verificados mergeados (#667 PRs #674/#690, #668 PRs #754/#757/#760/#762, #670 PRs #764/#766/#769/#780, base #796 = origin/main@b7446866). Rama `feat/669-glb-coherent-3d-representation` en worktree aislado. Un writer; 0 PRs abiertos al iniciar.
- Gate P0 CERRADO: fixture canónico con mesh real (bracket sólido L 70×54×18, MountFrame [25,30,15] rotZ90, constantes world hand-derived que replican la evidencia #668); escritor GLB Ruby determinista (convención canónica: metros/Y-up, (x,y,z)→(x,z,−y)); GLB commiteado producido por el host real (sha bc98563a…); host smoke 36 aserciones PASS con delta SKP↔GLB ≤2.5e-6 mm y mutantes ≥21 mm. Lectores TS (17) y Go (3) sobre los mismos bytes; contrato de validación compartido (17 casos).
- Backend (#669 P1): migración 000136 (provenance source_revision/exporter en revisions + pins glb en design_revision_hardware_assets), upload con derivación (source SKP del mismo asset), finalize fail-closed con validación estructural como evidencia, binding resuelve GLB co-representación (regla determinista latest-derived del SKP exacto), freeze congela GLB exacto (R1→G1 intacto tras re-export G2). Tests storage PG real 3/3 nuevos + api 1/1 (fixture real + corrupto + URI externa); OpenAPI 0 drift.
- Dominio/proyección (#669 P2): lookup/pins/proyección llevan glb {revisionId, sha256, sourceUnits, upAxis, sourceRevisionId}; snapshot histórico degrada a procedural con diagnóstico (nunca latest); freeze lleva pins glb. 5 tests nuevos (R1/R2, parcial honesto).
- Render (#669 P3/P4): GlbSceneCache (digest-keyed, normalización única horneada U·M_node), HardwareGlbMesh (estados honestos + fallback procedural, primitive estable — bug R3F de detach resuelto), grupo swap S; FurnitureScene3D vía contexto con provider API por sesión (authorize+grant+fetch) y seam de datos para guest/tests; Proyectar pasa visualAssetLookup (#668 pins); ModuleScene3D/Furniture3DViewer forwardean assemblies (mueble); mapper preserva mountFrame del binding.
- WebGL REAL (Point 12): extents canónicos, pairwise world IDÉNTICOS a la evidencia host, escala 1, det −1 (swap único), 0 console errors; suite visual 9/9.
- Evidence: domain 1582, ui 1955, visual 9/9, rake verify PASS (RBZ sha 7a7bbebc…), Go focalizados PASS, host smoke re-ejecutado en HEAD final (cae255a9). e2e hardware-3d actualizado a GLB real + caso negativo (pendiente de corrida browser completa).
- Pendiente explícito (Delivery: partial): comando host asistido interactivo (menú+upload derivado desde SketchUp — el writer y el contrato de upload están listos y probados), preview 3D en editor de herraje y Agregado (la vía ModuleScene3D ya expone assemblies), casos browser tenant/stale/FI-A-FI-B (estados cubiertos en unit), suites Go completas serializadas + CI del HEAD exacto.
- Delivery: partial (`Refs #669` + `Delivery: partial`). Sin merge ni cierre.
