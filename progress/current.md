...
- **PR #797 hardening documental (HEAD `5104cca822cc148979b802ccba6c0659c0b5e43d`)**: se documentan las correcciones de autoridad sin ampliar alcance: CNC explícito (`hasCncMachining` + `cncScope`) o DRAWING/BARCODE1 vacíos; DRAWING `D<hex12>` derivado de scope congelado + código de fabricación; CORE_MAT desde `ProductionCutRow.materialCode` sin tuning receptor; PALLET como nombre nominal sin cambiar posición serializada; edge flag sin `edgeBandCode` bloquea y sigue la limitación de un código de banda por pieza; rotación real del optimizador con `grain=0`/`allowRotationNoGrain` conserva lados físicos de pieza; cantidades inválidas cierran; r2/r3/r4, FUNCTION 92, perfiles/adapters y outputs cliente no se tocan. Evidencia observada para esta ronda: `pnpm --filter @granete/excel test` = 45 archivos / 534 PASS / 3 skips; `git diff --check` limpio. Pendiente en este HEAD: typecheck, selector, CI exact-head y verificación proporcional restante antes de publicar/cerrar.
