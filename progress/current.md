...
- **PR #797 hardening documental + corrección CORE_MAT (HEAD `32518fc47273f74af71520985953f68d9ca32bf3`)**: se documentan las correcciones de autoridad sin ampliar alcance: CNC explícito (`hasCncMachining` + `cncScope`) o DRAWING/BARCODE1 vacíos; DRAWING `D<hex12>` derivado de scope congelado + código de fabricación; CORE_MAT desde `ProductionCutRow.materialCode` sin tuning receptor y con validación fail-closed; PALLET como nombre nominal sin cambiar posición serializada; edge flag sin `edgeBandCode` bloquea y sigue la limitación de un código de banda por pieza; rotación real del optimizador con `grain=0`/`allowRotationNoGrain` conserva lados físicos de pieza; cantidades inválidas cierran; r2/r3/r4, FUNCTION 92, perfiles/adapters y outputs cliente no se tocan. Evidencia observada para este candidato: `pnpm --filter @granete/excel test` = 45 archivos / 534 PASS / 3 skips; `pnpm typecheck` PASS; diff check previo limpio. Selector autorizado `python3 scripts/verify_affected.py --base origin/main --budget-seconds 3600` sobre base `b7446866ed247677d8b8f83238cddbd96579db97` quedó bloqueado antes de gates por requerir `DATABASE_URL` aislado; jobs seleccionados: typescript, backend-go, sketchup-extension, proyectar-visual, foundation-postgres, organization-browser. Ninguno de esos gates corrió ni cuenta como PASS. Pendiente: selector con infraestructura aislada, CI exact-head y verificación proporcional restante antes de publicar/cerrar.
