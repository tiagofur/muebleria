...
- Evidencia (ver informe del PR para HEADs): Go storage sobre PG real 5/5 nuevos (`TestReleaseEngineeringLifecycleHttp`, `...P2DoesNotInheritP1`, `...IsolationAndExactness`, RED ×2) + suites domain/api completas PASS; `pnpm test` monorepo PASS (domain 1440 / storage 223 / excel 350+3 / desktop 17 / mobile 87 / UI 1906 / web 514), typecheck 7/7, `openapi:check` 0 drift; browser real (organization gate: docker PG + Go + Chromium) `engineering-state.spec.ts` 4/4 PASS — pendiente→iniciar→recarga→PDF/PTX no completa→completar→recarga, sin materiales/progreso físico/produced.
...
- Delivery: partial (PR `Refs #740` + `Delivery: partial`).
