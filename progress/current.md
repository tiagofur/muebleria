## #577 — final golden-path closure (ACTIVE; 2026-09-08)
...
- Objective: prove the ONE real React + Go + PostgreSQL golden path Q1 → R2 → Q2 accepted → P1 → frozen snapshot v2/routing → canonical material planning → warehouse reservations/materials release → server-derived exact PartExecutions → production stage. No second legacy release, no `rev-1`, no implicit latest, no mutable project/catalog recomputation.
- Prefer test/continuity closure over new product architecture. Mandatory audit of legacy seams (`engineeringLog.sentToProductionAt`, `project.productionRelease` blob, mapper revision fallbacks, mutable physical payloads). Relationship-bound modules stay fail-closed; fixture adapts, not architecture.
- Efficiency: classify delta (R2 if mostly E2E/test/wiring). Reuse still-valid #601/#603/#604/#606 evidence; no nested subagents; focused tests first.
- Merge-sensitive window: avoid unrelated merges to main until candidate publishes. No auto-merge; no #577 auto-close.
