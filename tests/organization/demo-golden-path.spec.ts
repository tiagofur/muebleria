...
test.describe.serial('DEMO golden path: Quote → SketchUp → DesignRevision → ProductionRelease (#644)', () => {
  ...
  test('stage 1b — accepted Q without a release creates no P, no Ingeniería entry and no release downloads', async ({ page }) => {
    ...
    // UI truth: Ingeniería does not claim this obra and nothing presents
    // itself as release content (no pinned release strip, no downloads).
    await loginToA(page);
  ...
  // turns 90° in place. Publishing the moved world must NOT create another
  // Q: compatibility comes from the backend's reconciliation classification
  // (zero commercial/manufacturing deltas), never from comparing revision
  // numbers, and approval/release stay explicit commands. The quote-side
  ...
  test('stage 11 — spatial-only move keeps the accepted commercial baseline (no new Q)', async () => {
    ...
    const r4 = await publishViaSketchUpContract(
      ...
      working.base_revision_id ?? track.r2Id, 'golden-publish-r4-spatial',
    );
