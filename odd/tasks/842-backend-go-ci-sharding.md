...
- Replaced that direct-admin job with `storage-shard-plan`, a visible four-way AST discovery/union/intersection/cardinality guard using the committed official-BEFORE timing fixture; `storage-shards`, a fail-fast-disabled `[1,2,3,4]` matrix that invokes only `scripts/backend-test-storage-shard.sh 4 <index> lpt`; and `backend-go-other`, which invokes the canonical runner for the exact `go list ./...` complement of `internal/storage`. `backend-go` remains the compatible aggregate name/title and requires plan + complete matrix + other backend proof.
