...
## PR #855 review correction — shard skip and AST cardinality proofs (2026-09-25)
- `ParseGoTestJSON` now derives the selected top-level root from every Go JSON test event. It fails closed on `skip` for a selected root, subtest, or nested subtest, and rejects events belonging to an unselected root. A successful verifier result therefore proves the wrapper's displayed `SKIP=0`; it is not a standalone hardcoded claim.
- The storage-plan verifier no longer accepts `--expected-count`: non-empty AST discovery is the membership authority, while complete union, exact once-only assignment, no unexpected roots, and no empty shard remain mandatory. The three operational shards stay fixed in the workflow. Current cardinality is still printed for observation, but a newly discovered root is valid when assigned exactly once.
- Strict-TDD RED: selected subtest/nested `skip` events were ignored by the original parser; Python plan tests failed because the old verifier required a manual count. GREEN: `cd backend-go && go test ./internal/testshard -count=1`; `python3 scripts/test_ci_storage_sharding.py`; `python3 scripts/test_factory_efficiency.py`; `bash -n scripts/backend-test-storage-shard.sh`; and static workflow proof for three shards with no `--expected-count` all passed.
- A local shard-1 smoke was started and executed against the isolated PostgreSQL runner, but the available command capture window ended before its summary. It is recorded as **NOT_CAPTURED**, not PASS; the exact-head Actions matrix remains the operational proof.
